...
public class RentalsViewModel extends ViewModel {
    ...
    public MutableLiveData<List<Rental>> getUserRentals(final Context context, final int user_id, final boolean active) {
        if (rentals == null)
            rentals = new MutableLiveData<>();

        RentalsRepository.getInstance().getUserRentals(context, user_id, active, rentals);
        return rentals;
    }
